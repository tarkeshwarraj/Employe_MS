...
const LoginPage = () => {
  ...
  const { url, setToken, role} = useStoreContext(); // Use the custom hook to get the URL
  const navigate = useNavigate(); //Initialize useHistory for redirection

  //This part will avoid to go in login page if already have token
  useEffect(() => {
  ...
  return (
    <>
      <div className="fixed inset-0 flex items-center justify-center">
        ...
        >
          <h2 className="text-lg font-semibold text-gray-700">{`ADMIN ${currState}`}</h2>
          {currState === 'Sign Up' && (
            <div>
            <input
              name="name"
